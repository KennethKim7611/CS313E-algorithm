...
#  Description: Recursion functions to corresponding scenarios
...
def splitArrayHelper(nums,index,balance):
    if index==len(nums):
        return balance==0
    if splitArrayHelper(nums, index+1,balance+nums[index]):
        return True
    return splitArrayHelper(nums,index+1,balance-nums[index])
...
def splitOdd10Helper(nums, index, g1, g2):
    if index==len(nums):
        return (g1%2==1 and g2%10==0) or (g2%2==1 and g1%10==0)
    if splitOdd10Helper(nums,index+1,g1+nums[index],g2):
        return True
    return splitOdd10Helper(nums,index+1,g1,g2+nums[index])
...
def split53Helper(nums,index,balance):
    if index==len(nums):
        return balance==0
    if nums[index]%5==0:
        return split53Helper(nums,index+1,balance+nums[index])
    if nums[index]%3==0:
        return split53Helper(nums,index+1,balance-nums[index])
    if split53Helper(nums,index+1,balance+nums[index]):
        return True
    return split53Helper(nums,index+1,balance-nums[index])
