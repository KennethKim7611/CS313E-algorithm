...
class Graph(object):
 ...
 # return an unvisited vertex adjacent to vertex v (index)
    def get_adj_unvisited_vertex(self, v):
        ...
        for i in range(nVert):
            if self.adjMat[v][i] > 0 \
                and not self.Vertices[i].was_visited():
                return i
        return -1
    ...
    def toposort(self):
        ...
        delete_list = []
        idx = 0
        # loop until graph's vertices length is 0
        while len(temp.Vertices) > 0:
            idx = 0
            # loop until index is less than graph's vertices length
            while idx < len(temp.Vertices):
                vertex = temp.Vertices[idx].label
                has_visit = False
                for i in range(len(temp.Vertices)):
                    # if adgNat is equal to 1, -> visited
                    if temp.adjMat[i][idx] == 1: 
                        has_visit = True
                        break
                # if not visited, add to topo_visit and delete from graph
                if not has_visit:
